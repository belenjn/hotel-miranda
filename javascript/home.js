const data = [
  {
    id: 1,
    room_name: "Leland Prairie",
    room_number: 20,
    floor_room: 4,
    bed_type: "Double bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "laborum culpa ipsum aliquip in eu et cupidatat anim enim enim dolore deserunt eu labore adipisicing laboris pariatur cupidatat Lorem id ex pariatur dolor esse labore",
    date_room: "2021-06-19 03:13:41",
    rate: 240,
    status: "false",
    image:
      "https://media.istockphoto.com/photos/hotel-room-suite-with-view-picture-id627892060?k=20&m=627892060&s=612x612&w=0&h=k6QY-qWNlFbvYhas82e_MoSXceozjrhhgp-krujsoDw=",
  },
  {
    id: 2,
    room_name: "Kuvalis Skyway",
    room_number: 84,
    floor_room: 9,
    bed_type: "Single bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "laborum culpa ipsum aliquip in eu et cupidatat anim enim enim dolore deserunt eu labore adipisicing laboris pariatur cupidatat Lorem id ex pariatur dolor esse labore",
    date_room: "2021-05-29 13:03:10",
    rate: 229,
    status: "false",
    image:
      "https://i2-prod.mylondon.news/incoming/article23105130.ece/ALTERNATES/s615/168289_WHotel-203000-London_IMG_05_0000jpeg.jpg",
  },
  {
    id: 3,
    room_name: "Pacocha Valleys",
    room_number: 11,
    floor_room: 4,
    bed_type: "Suite",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "dolor occaecat voluptate cupidatat tempor aliqua excepteur consequat aute deserunt cillum occaecat dolore incididunt adipisicing velit do",
    date_room: "2021-04-10 23:53:31",
    rate: 254,
    status: "true",
    image: "https://img.homejournal.com/202008/5f27752d94eeb.jpeg",
  },
  {
    id: 4,
    room_name: "Samantha Well",
    room_number: 46,
    floor_room: 4,
    bed_type: "Double superior",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "laborum culpa ipsum aliquip in eu et cupidatat anim enim enim dolore deserunt eu labore adipisicing laboris pariatur cupidatat Lorem id ex pariatur dolor esse labore",
    date_room: "2021-06-19 05:40:00",
    rate: 955,
    status: "true",
    image:
      "https://www.gannett-cdn.com/-mm-/05b227ad5b8ad4e9dcb53af4f31d7fbdb7fa901b/c=0-64-2119-1259/local/-/media/USATODAY/USATODAY/2014/08/13/1407953244000-177513283.jpg",
  },
  {
    id: 5,
    room_name: "Garret Bridge",
    room_number: 8,
    floor_room: 1,
    bed_type: "Single bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "laborum culpa ipsum aliquip in eu et cupidatat anim enim enim dolore deserunt eu labore adipisicing laboris pariatur cupidatat Lorem id ex pariatur dolor esse labore",
    date_room: "2022-10-31 10:01:41",
    rate: 296,
    status: "true",
    image:
      "https://media.istockphoto.com/photos/interior-of-a-modern-luxury-hotel-double-bed-bedroom-picture-id1163498940?k=20&m=1163498940&s=612x612&w=0&h=tUPidNW2ny095sCR97dur7cbrCnYpcjHbevUTJyB8Jc=",
  },
  {
    id: 6,
    room_name: "Kristian Circle",
    room_number: 78,
    floor_room: 1,
    bed_type: "Double Superior",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "pariatur labore incididunt exercitation irure mollit aliquip amet reprehenderit aliqua ipsum sint magna incididunt consectetur sunt irure ad amet occaecat incididunt eiusmod elit pariatur dolor velit consequat deserunt",
    date_room: "2022-09-01 07:23:21",
    rate: 698,
    status: "false",
    image:
      "https://cdna.artstation.com/p/assets/images/images/014/081/626/large/berktan-hatiboglu-modern-render1.jpg?1542377257",
  },
  {
    id: 7,
    room_name: "Gilbert Harbor",
    room_number: 36,
    floor_room: 3,
    bed_type: "Suite",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "pariatur exercitation officia adipisicing quis esse voluptate do officia occaecat et non veniam velit veniam enim",
    date_room: "2022-08-08 12:30:11",
    rate: 38,
    status: "true",
    image:
      "https://media.cntraveler.com/photos/53da5aa96dec627b149e509d/master/w_1024,h_768,c_limit/royal-ethiad-suite-jumeirah-abu-dhabi-2.jpg",
  },
  {
    id: 8,
    room_name: "Meredith Meadows",
    room_number: 93,
    floor_room: 7,
    bed_type: "Double bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "dolore cillum minim et elit tempor cillum pariatur consequat tempor ipsum amet qui veniam anim incididunt ea Lorem ullamco do nisi culpa",
    date_room: "2021-01-09 23:53:45",
    rate: 240,
    status: "false",
    image:
      "https://www.berjayahotel.com/sites/default/files/hotel-category-offers/kualalumpur/offers-room-berjaya-times-square-hotel-kuala-lumpur.jpg",
  },
  {
    id: 9,
    room_name: "Kian Dam",
    room_number: 86,
    floor_room: 10,
    bed_type: "Single bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "consequat nulla eu qui magna nostrud et voluptate cillum occaecat laboris duis deserunt officia officia ipsum cillum eiusmod est",
    date_room: "2022-12-04 02:35:01",
    rate: 831,
    status: "false",
    image:
      "https://i.pinimg.com/originals/b6/aa/91/b6aa915a8af1139561f0b9ec24a2e5af.jpg",
  },
  {
    id: 10,
    room_name: "Ledner Plaza",
    room_number: 70,
    floor_room: 4,
    bed_type: "Suite bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "fugiat adipisicing adipisicing ut culpa reprehenderit fugiat ut et adipisicing laborum exercitation Lorem nisi nostrud duis exercitation occaecat laboris ipsum amet excepteur incididunt quis sit",
    date_room: "2022-07-02 14:38:21",
    rate: 889,
    status: "false",
    image:
      "https://i.pinimg.com/originals/98/9d/fb/989dfbb168b61c8c77273094e6c07578.jpg",
  },
  {
    id: 11,
    room_name: "Jordi Plaza",
    room_number: 60,
    floor_room: 10,
    bed_type: "Double superior",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "laborum culpa ipsum aliquip in eu et cupidatat anim enim enim dolore deserunt eu labore adipisicing laboris pariatur cupidatat Lorem id ex pariatur dolor esse labore",
    date_room: "2022-11-05 16:10:51",
    rate: 62,
    status: "false",
    image:
      "https://media.istockphoto.com/photos/hotel-room-suite-with-view-picture-id627892060?k=20&m=627892060&s=612x612&w=0&h=k6QY-qWNlFbvYhas82e_MoSXceozjrhhgp-krujsoDw=",
  },
  {
    id: 12,
    room_name: "Leland Prairie",
    room_number: 20,
    floor_room: 4,
    bed_type: "Double bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "laborum culpa ipsum aliquip in eu et cupidatat anim enim enim dolore deserunt eu labore adipisicing laboris pariatur cupidatat Lorem id ex pariatur dolor esse labore",
    date_room: "2021-06-19 03:13:41",
    rate: 240,
    status: "false",
    image:
      "https://i2-prod.mylondon.news/incoming/article23105130.ece/ALTERNATES/s615/168289_WHotel-203000-London_IMG_05_0000jpeg.jpg",
  },
  {
    id: 13,
    room_name: "Kuvalis Skyway",
    room_number: 84,
    floor_room: 9,
    bed_type: "Single bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "id aliqua aute nisi magna esse non magna ad excepteur voluptate nisi nostrud elit sunt pariatur ex est in aliqua velit",
    date_room: "2021-05-29 13:03:10",
    rate: 229,
    status: "false",
    image: "https://img.homejournal.com/202008/5f27752d94eeb.jpeg",
  },
  {
    id: 14,
    room_name: "Pacocha Valleys",
    room_number: 11,
    floor_room: 4,
    bed_type: "Suite",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "dolor occaecat voluptate cupidatat tempor aliqua excepteur consequat aute deserunt cillum occaecat dolore incididunt adipisicing velit do",
    date_room: "2021-04-10 23:53:31",
    rate: 254,
    status: "true",
    image:
      "https://www.tokyuhotelsjapan.com/capitol-h/room_deluxeking_hollywoodtwin_images_05.jpg",
  },
  {
    id: 15,
    room_name: "Samantha Well",
    room_number: 46,
    floor_room: 4,
    bed_type: "Double superior",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "laborum culpa ipsum aliquip in eu et cupidatat anim enim enim dolore deserunt eu labore adipisicing laboris pariatur cupidatat Lorem id ex pariatur dolor esse labore",
    date_room: "2021-06-19 05:40:00",
    rate: 955,
    status: "true",
    image:
      "https://www.tokyuhotelsjapan.com/capitol-h/room/capitolsuite/images/room_cover_images_clubdeluxetwin.jpg",
  },
  {
    id: 16,
    room_name: "Garret Bridge",
    room_number: 8,
    floor_room: 1,
    bed_type: "Single bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "laborum culpa ipsum aliquip in eu et cupidatat anim enim enim dolore deserunt eu labore adipisicing laboris pariatur cupidatat Lorem id ex pariatur dolor esse labore",
    date_room: "2022-10-31 10:01:41",
    rate: 296,
    status: "true",
    image:
      "https://media.istockphoto.com/photos/interior-of-a-modern-luxury-hotel-double-bed-bedroom-picture-id1163498940?k=20&m=1163498940&s=612x612&w=0&h=tUPidNW2ny095sCR97dur7cbrCnYpcjHbevUTJyB8Jc=",
  },
  {
    id: 17,
    room_name: "Kristian Circle",
    room_number: 78,
    floor_room: 1,
    bed_type: "Double Superior",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "pariatur labore incididunt exercitation irure mollit aliquip amet reprehenderit aliqua ipsum sint magna incididunt consectetur sunt irure ad amet occaecat incididunt eiusmod elit pariatur dolor velit consequat deserunt",
    date_room: "2022-09-01 07:23:21",
    rate: 698,
    status: "false",
    image:
      "https://cdna.artstation.com/p/assets/images/images/014/081/626/large/berktan-hatiboglu-modern-render1.jpg?1542377257",
  },
  {
    id: 18,
    room_name: "Gilbert Harbor",
    room_number: 36,
    floor_room: 3,
    bed_type: "Suite",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "pariatur exercitation officia adipisicing quis esse voluptate do officia occaecat et non veniam velit veniam enim",
    date_room: "2022-08-08 12:30:11",
    rate: 38,
    status: "true",
    image:
      "https://media.cntraveler.com/photos/53da5aa96dec627b149e509d/master/w_1024,h_768,c_limit/royal-ethiad-suite-jumeirah-abu-dhabi-2.jpg",
  },
  {
    id: 19,
    room_name: "Meredith Meadows",
    room_number: 93,
    floor_room: 7,
    bed_type: "Double bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "dolore cillum minim et elit tempor cillum pariatur consequat tempor ipsum amet qui veniam anim incididunt ea Lorem ullamco do nisi culpa",
    date_room: "2021-01-09 23:53:45",
    rate: 240,
    status: "false",
    image:
      "https://www.berjayahotel.com/sites/default/files/hotel-category-offers/kualalumpur/offers-room-berjaya-times-square-hotel-kuala-lumpur.jpg",
  },
  {
    id: 20,
    room_name: "Kian Dam",
    room_number: 86,
    floor_room: 10,
    bed_type: "Single bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "consequat nulla eu qui magna nostrud et voluptate cillum occaecat laboris duis deserunt officia officia ipsum cillum eiusmod est",
    date_room: "2022-12-04 02:35:01",
    rate: 831,
    status: "false",
    image:
      "https://i.pinimg.com/originals/b6/aa/91/b6aa915a8af1139561f0b9ec24a2e5af.jpg",
  },
  {
    id: 21,
    room_name: "Ledner Plaza",
    room_number: 70,
    floor_room: 4,
    bed_type: "Suite bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "fugiat adipisicing adipisicing ut culpa reprehenderit fugiat ut et adipisicing laborum exercitation Lorem nisi nostrud duis exercitation occaecat laboris ipsum amet excepteur incididunt quis sit",
    date_room: "2022-07-02 14:38:21",
    rate: 889,
    status: "false",
    image:
      "https://i.pinimg.com/originals/98/9d/fb/989dfbb168b61c8c77273094e6c07578.jpg",
  },
  {
    id: 22,
    room_name: "Jordi Plaza",
    room_number: 60,
    floor_room: 10,
    bed_type: "Double superior",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "laborum culpa ipsum aliquip in eu et cupidatat anim enim enim dolore deserunt eu labore adipisicing laboris pariatur cupidatat Lorem id ex pariatur dolor esse labore",
    date_room: "2022-11-05 16:10:51",
    rate: 62,
    status: "false",
    image: "https://img.homejournal.com/202008/5f27752d94eeb.jpeg",
  },
  {
    id: 23,
    room_name: "Leland Prairie",
    room_number: 20,
    floor_room: 4,
    bed_type: "Double bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "laborum culpa ipsum aliquip in eu et cupidatat anim enim enim dolore deserunt eu labore adipisicing laboris pariatur cupidatat Lorem id ex pariatur dolor esse labore",
    date_room: "2021-06-19 03:13:41",
    rate: 240,
    status: "false",
    image:
      "https://media.istockphoto.com/photos/hotel-room-suite-with-view-picture-id627892060?k=20&m=627892060&s=612x612&w=0&h=k6QY-qWNlFbvYhas82e_MoSXceozjrhhgp-krujsoDw=",
  },
  {
    id: 24,
    room_name: "Kuvalis Skyway",
    room_number: 84,
    floor_room: 9,
    bed_type: "Single bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "id aliqua aute nisi magna esse non magna ad excepteur voluptate nisi nostrud elit sunt pariatur ex est in aliqua velit",
    date_room: "2021-05-29 13:03:10",
    rate: 229,
    status: "false",
    image:
      "https://i2-prod.mylondon.news/incoming/article23105130.ece/ALTERNATES/s615/168289_WHotel-203000-London_IMG_05_0000jpeg.jpg",
  },
  {
    id: 25,
    room_name: "Pacocha Valleys",
    room_number: 11,
    floor_room: 4,
    bed_type: "Suite",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "dolor occaecat voluptate cupidatat tempor aliqua excepteur consequat aute deserunt cillum occaecat dolore incididunt adipisicing velit do",
    date_room: "2021-04-10 23:53:31",
    rate: 254,
    status: "true",
    image: "https://img.homejournal.com/202008/5f27752d94eeb.jpeg",
  },
  {
    id: 26,
    room_name: "Samantha Well",
    room_number: 46,
    floor_room: 4,
    bed_type: "Double superior",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "laborum culpa ipsum aliquip in eu et cupidatat anim enim enim dolore deserunt eu labore adipisicing laboris pariatur cupidatat Lorem id ex pariatur dolor esse labore",
    date_room: "2021-06-19 05:40:00",
    rate: 955,
    status: "false",
    image:
      "https://www.gannett-cdn.com/-mm-/05b227ad5b8ad4e9dcb53af4f31d7fbdb7fa901b/c=0-64-2119-1259/local/-/media/USATODAY/USATODAY/2014/08/13/1407953244000-177513283.jpg",
  },
  {
    id: 27,
    room_name: "Garret Bridge",
    room_number: 8,
    floor_room: 1,
    bed_type: "Single bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "laborum culpa ipsum aliquip in eu et cupidatat anim enim enim dolore deserunt eu labore adipisicing laboris pariatur cupidatat Lorem id ex pariatur dolor esse labore",
    date_room: "2022-10-31 10:01:41",
    rate: 296,
    status: "false",
    image:
      "https://media.istockphoto.com/photos/interior-of-a-modern-luxury-hotel-double-bed-bedroom-picture-id1163498940?k=20&m=1163498940&s=612x612&w=0&h=tUPidNW2ny095sCR97dur7cbrCnYpcjHbevUTJyB8Jc=",
  },
  {
    id: 28,
    room_name: "Kristian Circle",
    room_number: 78,
    floor_room: 1,
    bed_type: "Double Superior",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "pariatur labore incididunt exercitation irure mollit aliquip amet reprehenderit aliqua ipsum sint magna incididunt consectetur sunt irure ad amet occaecat incididunt eiusmod elit pariatur dolor velit consequat deserunt",
    date_room: "2022-09-01 07:23:21",
    rate: 698,
    status: "true",
    image:
      "https://cdna.artstation.com/p/assets/images/images/014/081/626/large/berktan-hatiboglu-modern-render1.jpg?1542377257",
  },
  {
    id: 29,
    room_name: "Gilbert Harbor",
    room_number: 36,
    floor_room: 3,
    bed_type: "Suite",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "pariatur exercitation officia adipisicing quis esse voluptate do officia occaecat et non veniam velit veniam enim",
    date_room: "2022-08-08 12:30:11",
    rate: 38,
    status: "false",
    image:
      "https://media.cntraveler.com/photos/53da5aa96dec627b149e509d/master/w_1024,h_768,c_limit/royal-ethiad-suite-jumeirah-abu-dhabi-2.jpg",
  },
  {
    id: 30,
    room_name: "Meredith Meadows",
    room_number: 93,
    floor_room: 7,
    bed_type: "Double bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "dolore cillum minim et elit tempor cillum pariatur consequat tempor ipsum amet qui veniam anim incididunt ea Lorem ullamco do nisi culpa",
    date_room: "2021-01-09 23:53:45",
    rate: 240,
    status: "false",
    image:
      "https://www.berjayahotel.com/sites/default/files/hotel-category-offers/kualalumpur/offers-room-berjaya-times-square-hotel-kuala-lumpur.jpg",
  },
  {
    id: 31,
    room_name: "Kian Dam",
    room_number: 86,
    floor_room: 10,
    bed_type: "Single bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "consequat nulla eu qui magna nostrud et voluptate cillum occaecat laboris duis deserunt officia officia ipsum cillum eiusmod est",
    date_room: "2022-12-04 02:35:01",
    rate: 831,
    status: "false",
    image:
      "https://i.pinimg.com/originals/b6/aa/91/b6aa915a8af1139561f0b9ec24a2e5af.jpg",
  },
  {
    id: 32,
    room_name: "Ledner Plaza",
    room_number: 70,
    floor_room: 4,
    bed_type: "Suite bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "fugiat adipisicing adipisicing ut culpa reprehenderit fugiat ut et adipisicing laborum exercitation Lorem nisi nostrud duis exercitation occaecat laboris ipsum amet excepteur incididunt quis sit",
    date_room: "2022-07-02 14:38:21",
    rate: 889,
    status: "true",
    image:
      "https://media.istockphoto.com/photos/hotel-room-suite-with-view-picture-id627892060?k=20&m=627892060&s=612x612&w=0&h=k6QY-qWNlFbvYhas82e_MoSXceozjrhhgp-krujsoDw=",
  },
  {
    id: 33,
    room_name: "Jordi Plaza",
    room_number: 60,
    floor_room: 10,
    bed_type: "Double superior",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "laborum culpa ipsum aliquip in eu et cupidatat anim enim enim dolore deserunt eu labore adipisicing laboris pariatur cupidatat Lorem id ex pariatur dolor esse labore",
    date_room: "2022-11-05 16:10:51",
    rate: 62,
    status: "true",
    image:
      "https://i.pinimg.com/originals/98/9d/fb/989dfbb168b61c8c77273094e6c07578.jpg",
  },
  {
    id: 34,
    room_name: "Leland Prairie",
    room_number: 20,
    floor_room: 4,
    bed_type: "Double bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description: "id proident non excepteur occaecat",
    date_room: "2021-06-19 03:13:41",
    rate: 240,
    status: "false",
    image:
      "https://i2-prod.mylondon.news/incoming/article23105130.ece/ALTERNATES/s615/168289_WHotel-203000-London_IMG_05_0000jpeg.jpg",
  },
  {
    id: 35,
    room_name: "Kuvalis Skyway",
    room_number: 84,
    floor_room: 9,
    bed_type: "Single bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "id aliqua aute nisi magna esse non magna ad excepteur voluptate nisi nostrud elit sunt pariatur ex est in aliqua velit",
    date_room: "2021-05-29 13:03:10",
    rate: 229,
    status: "false",
    image:
      "https://www.tokyuhotelsjapan.com/capitol-h/room/capitolsuite/images/room_cover_images_clubdeluxetwin.jpg",
  },
  {
    id: 36,
    room_name: "Pacocha Valleys",
    room_number: 11,
    floor_room: 4,
    bed_type: "Suite",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "dolor occaecat voluptate cupidatat tempor aliqua excepteur consequat aute deserunt cillum occaecat dolore incididunt adipisicing velit do",
    date_room: "2021-04-10 23:53:31",
    rate: 254,
    status: "true",
    image:
      "https://media.istockphoto.com/photos/interior-of-a-modern-luxury-hotel-double-bed-bedroom-picture-id1163498940?k=20&m=1163498940&s=612x612&w=0&h=tUPidNW2ny095sCR97dur7cbrCnYpcjHbevUTJyB8Jc=",
  },
  {
    id: 37,
    room_name: "Samantha Well",
    room_number: 46,
    floor_room: 4,
    bed_type: "Double superior",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "laborum culpa ipsum aliquip in eu et cupidatat anim enim enim dolore deserunt eu labore adipisicing laboris pariatur cupidatat Lorem id ex pariatur dolor esse labore",
    date_room: "2021-06-19 05:40:00",
    rate: 955,
    status: "true",
    image:
      "https://media.cntraveler.com/photos/53da5aa96dec627b149e509d/master/w_1024,h_768,c_limit/royal-ethiad-suite-jumeirah-abu-dhabi-2.jpg",
  },
  {
    id: 38,
    room_name: "Garret Bridge",
    room_number: 8,
    floor_room: 1,
    bed_type: "Single bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "laborum culpa ipsum aliquip in eu et cupidatat anim enim enim dolore deserunt eu labore adipisicing laboris pariatur cupidatat Lorem id ex pariatur dolor esse labore",
    date_room: "2022-10-31 10:01:41",
    rate: 296,
    status: "true",
    image:
      "https://media.cntraveler.com/photos/53da5aa96dec627b149e509d/master/w_1024,h_768,c_limit/royal-ethiad-suite-jumeirah-abu-dhabi-2.jpg",
  },
  {
    id: 39,
    room_name: "Kristian Circle",
    room_number: 78,
    floor_room: 1,
    bed_type: "Double Superior",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "pariatur labore incididunt exercitation irure mollit aliquip amet reprehenderit aliqua ipsum sint magna incididunt consectetur sunt irure ad amet occaecat incididunt eiusmod elit pariatur dolor velit consequat deserunt",
    date_room: "2022-09-01 07:23:21",
    rate: 698,
    status: "false",
    image:
      "https://www.berjayahotel.com/sites/default/files/hotel-category-offers/kualalumpur/offers-room-berjaya-times-square-hotel-kuala-lumpur.jpg",
  },
  {
    id: 40,
    room_name: "Gilbert Harbor",
    room_number: 36,
    floor_room: 3,
    bed_type: "Suite",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "pariatur exercitation officia adipisicing quis esse voluptate do officia occaecat et non veniam velit veniam enim",
    date_room: "2022-08-08 12:30:11",
    rate: 38,
    status: "true",
    image:
      "https://i.pinimg.com/originals/b6/aa/91/b6aa915a8af1139561f0b9ec24a2e5af.jpg",
  },
  {
    id: 41,
    room_name: "Meredith Meadows",
    room_number: 93,
    floor_room: 7,
    bed_type: "Double bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "dolore cillum minim et elit tempor cillum pariatur consequat tempor ipsum amet qui veniam anim incididunt ea Lorem ullamco do nisi culpa",
    date_room: "2021-01-09 23:53:45",
    rate: 240,
    status: "false",
    image:
      "https://media.istockphoto.com/photos/interior-of-a-modern-luxury-hotel-double-bed-bedroom-picture-id1163498940?k=20&m=1163498940&s=612x612&w=0&h=tUPidNW2ny095sCR97dur7cbrCnYpcjHbevUTJyB8Jc=",
  },
  {
    id: 42,
    room_name: "Kian Dam",
    room_number: 86,
    floor_room: 10,
    bed_type: "Single bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "consequat nulla eu qui magna nostrud et voluptate cillum occaecat laboris duis deserunt officia officia ipsum cillum eiusmod est",
    date_room: "2022-12-04 02:35:01",
    rate: 831,
    status: "false",
    image:
      "https://cdna.artstation.com/p/assets/images/images/014/081/626/large/berktan-hatiboglu-modern-render1.jpg?1542377257",
  },
  {
    id: 43,
    room_name: "Ledner Plaza",
    room_number: 70,
    floor_room: 4,
    bed_type: "Suite bed",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "fugiat adipisicing adipisicing ut culpa reprehenderit fugiat ut et adipisicing laborum exercitation Lorem nisi nostrud duis exercitation occaecat laboris ipsum amet excepteur incididunt quis sit",
    date_room: "2022-07-02 14:38:21",
    rate: 889,
    status: "false",
    image:
      "https://www.berjayahotel.com/sites/default/files/hotel-category-offers/kualalumpur/offers-room-berjaya-times-square-hotel-kuala-lumpur.jpg",
  },
  {
    id: 44,
    room_name: "Jordi Plaza",
    room_number: 60,
    floor_room: 10,
    bed_type: "Double superior",
    facilities:
      "AC, Shower, Double Bed, Towel, Bathup, Coffee Set, LED TV, Wifi",
    description:
      "laborum culpa ipsum aliquip in eu et cupidatat anim enim enim dolore deserunt eu labore adipisicing laboris pariatur cupidatat Lorem id ex pariatur dolor esse labore",
    date_room: "2022-11-05 16:10:51",
    rate: 62,
    status: "false",
    image:
      "https://i2-prod.mylondon.news/incoming/article23105130.ece/ALTERNATES/s615/168289_WHotel-203000-London_IMG_05_0000jpeg.jpg",
  },
];
const takeATourButton = document.querySelector("#tour__button");

takeATourButton.addEventListener("click", function () {
  window.scrollTo("2400", "1300");
});

const learnButton = document.querySelector("#learn__button");

learnButton.addEventListener("click", function () {
  window.scrollTo("3000", "2500");
});


const button = document.querySelector("#button");

button.addEventListener("click", function () {
  window.location.href = "rooms.html";
});

const buttonVideo = document.querySelector("#button__video");

buttonVideo.addEventListener("click", function () {
  window.location.href = "rooms.html";
});


const inputs = document.getElementsByClassName("form__input");

const input1 = inputs[0];

const input2 = inputs[1];

const buttonAvailability = document.getElementById("availability");

let datesOfRooms = [];

for (let index = 0; index < data.length; index++) {
  const dateRoom = Date.parse(data[index].date_room.slice(0, 10));
  datesOfRooms.push(dateRoom);
}


let eventTargetValue1;
let eventTargetValue2;

input1.addEventListener("change", function (e) {
  eventTargetValue1 = Date.parse(e.target.value.toString());
});

input2.addEventListener("change", function (e) {
  eventTargetValue2 = Date.parse(e.target.value.toString());
});

buttonAvailability.addEventListener("click", function (e) {
  e.preventDefault();

  const value1 = datesOfRooms.find((date) =>
    date === eventTargetValue1 ? date : ""
  );
  const value2 = datesOfRooms.find((date) =>
    date === eventTargetValue2 ? date : ""
  );

  value1 && value2
    ? (alert("No rooms available, try other dates"),
      (input1.value = ""),
      (input2.value = ""))
    : alert(
        "If you want to navigate to the available rooms, click in Accept",
        setTimeout(() => {
          window.location.href = "rooms.html";
        }, 3000)
      );
});
